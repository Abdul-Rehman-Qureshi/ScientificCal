...
double anlogBasee() {
	...
	cout<<"Anti Log with base e of "<< a <<" is = "<< anlogBasee;
	return anlogBasee;
}
int table() {
	int table ;
	...
	while (true) {
		...
		cin >> a;
		cout << "enter the number where u want to end the table:" << endl;
		cin >> b;
        if (cin.fail()) {
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
...
float invssin() {
	...
	invssin = asin(a);
	cout<< "Inverse of sin " << a <<" is = "<< invssin;
	return invssin;
}
float invscos() {
	float PI = 3.14159265;
	...
	invscos = acos(a);
	cout<<"Inverse of Cos " << a <<" is = "<<invscos;
	return invscos;
}
float invstan() {
	float PI = 3.14159265;
	...
	invstan = atan(a);
	cout<<"Inverse of Tan " << a << " is = "<<invstan;
	return invstan;
}
void history()
{
...
int main() {
	...
	cout << "6: Square root\t\t" << "14: Inverse of Tan" << "\t\t20: Calculation history" << endl;
